...
const List<_AttrSection> _kAttributeSections = [
  _AttrSection(
    'Nhu cầu sử dụng',
    ProductFilterQueryKeys.usageNeeds,
    [
      'Chơi game',
      'Pin trâu',
      'Dung lượng lớn',
  ...
  _AttrSection(
    'Chip xử lí',
    ProductFilterQueryKeys.chipset,
    [
      'Snapdragon',
      'Apple A',
      'Mediatek Dimensity',
  ...
  _AttrSection(
    'Loại điện thoại',
    ProductFilterQueryKeys.operatingSystem,
    [
      'iPhone (iOS)',
      'Android',
  ...
  _AttrSection(
    'Bộ nhớ trong',
    ProductFilterQueryKeys.internalStorage,
    [
      'Dưới 64 GB',
      '64 GB',
      '128 GB',
  ...
  _AttrSection(
    'Tính năng đặc biệt',
    ProductFilterQueryKeys.specialFeatures,
    [
      'Sạc không dây',
      'Bảo mật vân tay',
      'Nhận diện khuôn mặt',
  ...
  _AttrSection(
    'Tính năng camera',
    ProductFilterQueryKeys.cameraFeatures,
    [
      'Chụp xóa phông',
      'Chụp góc rộng',
      'Quay video 4K',
  ...
  _AttrSection(
    'Tần số quét',
    ProductFilterQueryKeys.refreshRate,
    ['60Hz', '120Hz', '90Hz', 'Từ 144Hz trở lên'],
  ),
  _AttrSection(
    'Kích thước màn hình',
    ProductFilterQueryKeys.screenSize,
    ['Trên 6 inch', 'Dưới 6 inch'],
  ),
  _AttrSection(
    'Kiểu màn hình',
    ProductFilterQueryKeys.screenType,
    [
      'Tai thỏ',
...
class _FilterPopUpState extends State<FilterPopUp> {
  ...
  void initState() {
    ...
    for (final section in _kAttributeSections) {
      ...
      _selectedByKey[section.queryKey] = {
  ...
  Future<void> _loadHighestPrice() async {
    ...
    final snappedStart = PriceRangeFilterHelper.snapToStep(minClamped.toDouble());
  ...
  void _applyParsedFieldsToRange() {
    final minD = PriceRangeFilterHelper.parseLooseMoneyToDecimal(_minField.text) ??
        Decimal.zero;
    final maxD = PriceRangeFilterHelper.parseLooseMoneyToDecimal(_maxField.text) ??
        Decimal.zero;
    ...
    final snappedStart =
        PriceRangeFilterHelper.snapToStep(minV).clamp(0.0, _maxAvailablePrice);
    final snappedEnd = PriceRangeFilterHelper.snapToStep(maxV)
          ...
          if (_loadingPrice)
            ...
            Row(
              ...
              children: [
                ...
                AppText(
                  text: PriceRangeFilterHelper.formatVnPrice(_maxAvailablePrice),
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
                  color: AppColors.gray600,
                ),
            ...
            SliderTheme(
              ...
              child: RangeSlider(
                ...
                onChanged: (values) {
                  final snappedStart = PriceRangeFilterHelper.snapToStep(values.start)
                      .clamp(0.0, _maxAvailablePrice)
                      .toDouble();
                  final snappedEnd = PriceRangeFilterHelper.snapToStep(values.end)
                      .clamp(snappedStart, _maxAvailablePrice)
                      .toDouble();
                  setState(() {
                    _selectedRange = RangeValues(snappedStart, snappedEnd);
  ...
  Widget _moneyField(TextEditingController c) {
    return TextField(
      ...
      inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'[\d\.\sđ]'))],
      style: TextStyle(
        fontSize: 14,
        fontWeight: FontWeight.w600,
        color: AppColors.gray900,
      ...
      children: section.options
          .map(
            (label) => _selectableChip(
              label: label,
              selected: _selectedByKey[section.queryKey]?.contains(label) ?? false,
              onTap: () {
                setState(() {
                  final set =
                      _selectedByKey.putIfAbsent(section.queryKey, () => {});
